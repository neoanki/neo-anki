...
> **Pre-1.0 status:** Neo Anki is an actively developed preview, not a drop-in Anki replacement. The source tree also contains browser, mobile, sync, and marketplace implementations that have additional release gates. Check the notes attached to a published release for the behavior in its binaries, and keep independent backups of important collections.
...
- Electron desktop application with integrated macOS window chrome, native menus and shortcuts, a study queue and card browser, SQLite persistence, content-addressed media, recovery backups, native backup restore/export, and light/dark themes. The current persistence projection has known rich-field and overlapping-save risks; see the [claim evidence register](docs/claim-evidence.md).
...
- Installable `.neoanki-extension` packages use SDK 2 exclusively. Packages are signed, logic runs in workers, UI runs in sandboxed iframes, and schema/SDK 1 packages are rejected before installation. Reload/re-enable capability continuity still has a known lifecycle gap.
- The current source tree lets desktop, web, and mobile browse the public review-gated [extension marketplace](docs/extension-marketplace.md). [NeoAnki TTS 2.0.1](https://github.com/neoanki/neoanki-tts/releases/tag/v2.0.1) is its first production listing. Only desktop has an install runtime; marketplace integration is newer than the published v0.1.5 binary.
