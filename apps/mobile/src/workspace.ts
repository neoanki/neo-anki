...
export const addNote = (input: WorkspaceDocumentV4, noteTypeId: string, deckId: string, fields: Record<string, string>, tags: string[] = []) => {
  ...
  const requiredFieldIds = type.kind === 'cloze' ? type.fieldIds.slice(0, 1) : type.fieldIds.slice(0, Math.min(2, type.fieldIds.length))
  if (!requiredFieldIds.length || requiredFieldIds.some((fieldId) => !normalizedFields[fieldId])) throw new Error(type.kind === 'cloze' ? 'Enter cloze content before adding this knowledge item.' : 'Enter both a prompt and an answer before adding this knowledge item.')
