...
test('desktop app persists the workspace to disk across restarts', async () => {
  ...
  try {
    ...
    await expect(firstWindow.getByRole('heading', { name: /make memory fit your life/i })).toBeVisible()
    ...
    await expect(secondWindow.getByRole('heading', { name: /make memory fit your life/i })).toBeVisible()
