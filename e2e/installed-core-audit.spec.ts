...
test('released desktop artifact supports the blank core journey', async () => {
  ...
  try {
    ...
    await window.getByLabel('Prompt').fill('What is the release artifact audit phrase?')
    await window.getByLabel('Answer').fill('Packaged journey verified')
    ...
    await expect(window.getByLabel('Front')).toHaveValue('What is the release artifact audit phrase?')
    await window.getByLabel('Back').fill('Packaged journey verified after editing')
...
test('released desktop artifact blocks writes and preserves a corrupt workspace', async () => {
  ...
  try {
    ...
    await window.getByRole('button', { name: /create workspace/i }).click()
    await window.getByRole('button', { name: /add your first knowledge item/i }).click()
    await window.getByLabel('Prompt').fill('This content must survive as preserved corrupt source')
    await window.getByLabel('Answer').fill('Preserved')
    await window.getByRole('button', { name: /add knowledge/i }).click()
    await expect(window.getByRole('status')).toContainText(/added|queue/i)
    await application.close()
    application = undefined
