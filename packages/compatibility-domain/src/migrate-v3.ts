...
export const migrateWorkspaceV3ToV4 = (legacy: LegacyWorkspaceV3, workspaceId: string = crypto.randomUUID()): WorkspaceV4 => {
  ...
  const envelopes: SourceEnvelope[] = legacy.items.map((item) => ({
    ...
    opaque: safeOpaque({ extensionData: item.extensionData || {}, legacy: { citations: item.citations || [], mediaIds: item.mediaIds || [], occlusions: item.occlusions || [], source: item.source, provenance: item.provenance, extensionData: item.extensionData || {} } }),
  }))
  envelopes.push(...legacy.cards.map((card) => ({ id: `source:v3:card:${card.id}`, revision: 1, createdAt: now, updatedAt: now, profileId, format: 'neo-v3' as const, sourceId: card.id, schemaVersion: '3', opaque: safeOpaque({ legacy: { variant: card.variant, occlusionId: card.occlusionId, promptData: card.promptData, estimatedSeconds: card.estimatedSeconds } }) })))
  envelopes.push(...legacy.reviews.map((review) => ({ id: `source:v3:review:${review.id}`, revision: 1, createdAt: now, updatedAt: now, profileId, format: 'neo-v3' as const, sourceId: review.id, schemaVersion: '3', opaque: safeOpaque({ legacy: { deviceId: (review as LegacyReview & { deviceId?: string }).deviceId, rawDurationSeconds: review.rawDurationSeconds, previousDue: review.previousDue, nextDue: review.nextDue, previousCard: review.previousCard, previousEstimatedSeconds: review.previousEstimatedSeconds, previousCardState: review.previousCardState, siblingChanges: review.siblingChanges } }) })))
  envelopes.push(...legacy.assets.map((asset) => ({ id: `source:v3:media:${asset.id}`, revision: 1, createdAt: now, updatedAt: now, profileId, format: 'neo-v3' as const, sourceId: asset.id, schemaVersion: '3', opaque: safeOpaque({ legacy: { altText: asset.altText || '' } }) })))
  envelopes.push({ id: 'source:v3:workspace', revision: 1, createdAt: now, updatedAt: now, profileId, format: 'neo-v3', sourceId: 'workspace', schemaVersion: '3', opaque: {} })
