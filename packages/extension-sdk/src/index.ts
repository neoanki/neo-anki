...
export type ExtensionPermissionV2 = 'study:read' | 'study:signals' | 'content:read' | 'content:patch-own' | 'media:create' | 'network:fetch' | 'secrets:device' | 'config:sync' | 'ui:settings' | 'ui:review' | 'ui:page'
...
export interface ExtensionManifestV2 {
  ...
  uiEntries?: Array<{ id: string; surface: 'settings' | 'review' | 'page'; entry: string }>
...
export type WorkerContributionRequest = { type: 'planning-signals'; request: ScopedStudyDto } | { type: 'command'; requestId: string; commandId: string; payload: unknown } | { type: 'cancel'; operationId: string }
...
export type ExtensionHostMethodV2 = 'applyPatch' | 'createMedia' | 'fetch' | 'cancel' | 'secrets.read' | 'secrets.mutate' | 'config.read' | 'config.write' | 'content.listNotes'
...
const workerRequestId = (request: WorkerContributionRequest) => request.type === 'planning-signals' ? request.request.requestId : request.type === 'command' ? request.requestId : request.operationId
