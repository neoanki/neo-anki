...
export const createSandboxedExtensionUiV2 = (manifest: ExtensionManifestV2, contributionId: string, entryUrl: string, init: Omit<SandboxedUiInit, 'type' | 'extensionId' | 'contributionId'>, hostCall?: (name: string, payload: unknown) => Promise<unknown>, onLifecycle?: (event: SandboxedUiLifecycleEventV2) => void): SandboxedUiRuntimeV2 => {
  ...
  const frameDocument = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src file: neoanki-extension: blob: http://127.0.0.1:* http://localhost:*; style-src 'unsafe-inline'; img-src data: blob:; media-src blob: neoanki-media:; connect-src 'none'; font-src 'none'; base-uri 'none'; form-action 'none'"><style>html,body,#root{margin:0;min-height:100%;font:16px/1.5 system-ui,sans-serif}</style></head><body><div id="root"></div><script type="module" src="${escapedEntry}"></script></body></html>`
