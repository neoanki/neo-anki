...
export const KnowledgeItemEditor = ({ item, onClose }: { item: KnowledgeItem; onClose: () => void }) => {
  ...
  const isBasic = draft.contentModel?.contentTypeName === 'Basic' || draft.contentModel?.contentTypeName === 'Neo Basic'
  ...
  const save = async (event: React.FormEvent) => {
    ...
    const ordered = draft.contentModel?.fields.slice().sort((left, right) => left.ordinal - right.ordinal)
    setSaving(true)
    setSaveError('')
    try {
      await updateItem(item.id, { prompt: ordered?.[0]?.value ?? draft.prompt, answer: ordered?.[1]?.value ?? ordered?.[0]?.value ?? draft.answer, context: ordered ? ordered.slice(2).map((field) => field.value).filter(Boolean).join('\n') : draft.context, collection: draft.collection, tags: draft.tags, source: draft.source, citations: draft.citations, contentModel: draft.contentModel })
      onClose()
    } catch (error) {
  ...
  return (
    <div className="scrim" role="presentation" onMouseDown={onBackdropMouseDown}>
      <form ref={dialogRef} tabIndex={-1} className="edit-dialog" role="dialog" aria-modal="true" aria-labelledby="edit-title" aria-busy={saving} onSubmit={save}>
        ...
        {draft.contentModel ? <fieldset className="sub-editor note-field-editor"><legend>{isBasic ? 'Knowledge content' : `Fields · ${draft.contentModel.contentTypeName}`}</legend>{draft.contentModel.fields.slice().sort((left, right) => left.ordinal - right.ordinal).map((field) => {
          const label = isBasic && field.ordinal < 3 ? ['Prompt', 'Answer', 'Context'][field.ordinal] : field.name
          return <div className="form-field" key={field.id}><label htmlFor={`edit-field-${field.id}`}>{label}{isBasic && field.ordinal === 2 && <span aria-hidden="true"> Optional</span>}</label><textarea id={`edit-field-${field.id}`} rows={field.ordinal === 0 ? 4 : 3} value={field.value} onChange={(event) => updateNamedField(field.id, event.target.value)} required={isBasic && field.ordinal < 2} /></div>
