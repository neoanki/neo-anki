...
export const importCsvText = (text: string): ImportSummary => {
  ...
  for (const [offset, values] of rows.slice(1).entries()) {
    ...
    const variants = (record.variants || 'forward').split('|').filter((variant): variant is PromptVariant => ['forward', 'reverse', 'cloze', 'typed', 'image-occlusion', 'audio'].includes(variant))
    items.push({
