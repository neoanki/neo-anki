...
import { extensionPromptTypesV2, extensionUiContributionsV2 } from '../extensions/v2/registry'
...
export const CreatePage = () => {
  ...
  const promptTypes: Array<{ value: PromptVariant; label: string }> = [
    ...
    ...extensionPromptTypesV2().map((promptType) => ({ value: promptType.id, label: promptType.label })),
  ]
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setSaving(true)
    try { await addItem({ prompt, answer, context, collection, tags: tags.split(',').map((tag) => tag.trim()).filter(Boolean), citations: citations.filter((citation) => citation.title.trim()).map((citation) => ({ ...citation, url: citation.url?.trim() || undefined })), assets, occlusions: occlusions.map(normalizeRect), variants }) }
  ...
  return <div className="page create-page">
    ...
    <div className="create-layout">
      <form className="editor-card" onSubmit={submit}>
        ...
        <div className="editor-footer"><p><Sparkles size={17}/> New prompts enter only when the time forecast has room.</p><button className="primary-button" type="submit" disabled={saving || !prompt.trim() || !answer.trim() || !variants.length}><Plus size={19}/> {saving ? 'Adding…' : 'Add knowledge'}</button></div>
