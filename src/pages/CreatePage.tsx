  ...
  return <div className="page create-page">
    ...
    <div className="create-layout">
      <form className="editor-card" onSubmit={submit}>
        <label className="form-field"><span>Content type</span><select value={contentTypeId} onChange={(event) => selectContentType(event.target.value)} disabled={!document}>{document?.workspace.noteTypes.map((type) => <option value={type.id} key={type.id}>{type.name}</option>)}</select><small>Fields and card layouts are managed in Settings → Card templates.</small></label>
