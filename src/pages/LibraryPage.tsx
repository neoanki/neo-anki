import { Archive, ArchiveRestore, CalendarClock, Edit3, Eye, Filter, Flag, MoreHorizontal, Play, Plus, Search, Trash2, Undo2, X } from 'lucide-react'
...
const EditDialog = ({ item, onClose }: { item: KnowledgeItem; onClose: () => void }) => {
  ...
  const [dialogRef, requestClose, onBackdropMouseDown] = useModalDialog<HTMLFormElement>(onClose, { dirty, dirtyMessage: 'Discard unsaved edits to this knowledge item?' })
  const save = (event: React.FormEvent) => {
    event.preventDefault()
    const ordered = draft.noteModel?.fields.slice().sort((left, right) => left.ordinal - right.ordinal)
    updateItem(item.id, { prompt: ordered?.[0]?.value ?? draft.prompt, answer: ordered?.[1]?.value ?? ordered?.[0]?.value ?? draft.answer, context: ordered ? ordered.slice(2).map((field) => field.value).filter(Boolean).join('\n') : draft.context, collection: draft.collection, tags: draft.tags, source: draft.source, citations: draft.citations, noteModel: draft.noteModel })
    onClose()
  }
  return (
    <div className="scrim" role="presentation" onMouseDown={onBackdropMouseDown}>
      <form ref={dialogRef} tabIndex={-1} className="edit-dialog" role="dialog" aria-modal="true" aria-labelledby="edit-title" onSubmit={save}>
        <div className="panel-header"><div><p className="eyebrow">Knowledge item</p><h2 id="edit-title">Edit content</h2></div><button type="button" className="icon-button" onClick={requestClose} aria-label="Close editor"><X size={20} /></button></div>
        {draft.noteModel ? <fieldset className="sub-editor note-field-editor"><legend>Named fields · {draft.noteModel.noteTypeName}</legend>{draft.noteModel.fields.slice().sort((left, right) => left.ordinal - right.ordinal).map((field) => <div className="form-field" key={field.id}><label htmlFor={`edit-field-${field.id}`}>{field.name}</label><textarea id={`edit-field-${field.id}`} rows={field.ordinal === 0 ? 4 : 3} value={field.value} onChange={(event) => setDraft({ ...draft, noteModel: { ...draft.noteModel!, fields: draft.noteModel!.fields.map((candidate) => candidate.id === field.id ? { ...candidate, value: event.target.value } : candidate) } })} /></div>)}</fieldset> : <><div className="form-field"><label htmlFor="edit-prompt">Prompt</label><textarea id="edit-prompt" rows={4} value={draft.prompt} onChange={(event) => setDraft({ ...draft, prompt: event.target.value })} /></div><div className="form-field"><label htmlFor="edit-answer">Answer</label><textarea id="edit-answer" rows={3} value={draft.answer} onChange={(event) => setDraft({ ...draft, answer: event.target.value })} /></div><div className="form-field"><label htmlFor="edit-context">Context</label><textarea id="edit-context" rows={2} value={draft.context} onChange={(event) => setDraft({ ...draft, context: event.target.value })} /></div></>}
        ...
        <div className="dialog-actions"><button type="button" className="secondary-button" onClick={requestClose}>Cancel</button><button className="primary-button" type="submit" disabled={!dirty}>Save changes</button></div>
      </form>
    </div>
  )
}
  ...
  return (
    <div className="page library-page">
      ...
      <div className="library-toolbar">
        <div className="library-mode" role="group" aria-label="Browse by"><button type="button" className={mode === 'notes' ? 'active' : ''} aria-pressed={mode === 'notes'} onClick={() => { setMode('notes'); setSelected(new Set()); setVisibleCount(LIBRARY_PAGE_SIZE) }}>Notes</button><button type="button" className={mode === 'cards' ? 'active' : ''} aria-pressed={mode === 'cards'} onClick={() => { setMode('cards'); setSelected(new Set()); setVisibleCount(LIBRARY_PAGE_SIZE) }}>Cards</button></div>
      ...
      {selected.size > 0 && <div className="bulk-toolbar" role="toolbar" aria-label="Bulk actions">
        ...
        <button className="secondary-button compact" onClick={() => setCardsSuspended(selectedCards.map((card) => card.id), true)}>Suspend cards</button>
        <button className="secondary-button compact" onClick={() => setCardsSuspended(selectedCards.map((card) => card.id), false)}>Resume cards</button>
        ...
        <label className="bulk-flag"><Flag size={16} aria-hidden="true" /><span className="visually-hidden">Set flag on selected cards</span><select aria-label="Set flag on selected cards" defaultValue="" onChange={(event) => { if (event.target.value !== '') setCardsFlag(selectedCards.map((card) => card.id), Number(event.target.value) as 0 | 1 | 2 | 3 | 4 | 5 | 6 | 7); event.currentTarget.value = '' }}><option value="" disabled>Set flag</option><option value="0">No flag</option>{Array.from({ length: 7 }, (_, index) => <option key={index + 1} value={index + 1}>Flag {index + 1}</option>)}</select></label>
        <label className="bulk-flag"><span className="visually-hidden">Move selected {mode} to deck</span><select aria-label={`Move selected ${mode} to deck`} disabled={bulkBusy} defaultValue="" onChange={(event) => { const control = event.currentTarget; const name = control.value; if (name) { if (mode === 'notes') updateItemsBulk([...selected], { collection: name }); else { setBulkBusy(true); void setCardsDeck([...selected], name).then(() => setBulkMessage(`Moved ${selectedCards.length} selected ${selectedCards.length === 1 ? 'card' : 'cards'} to ${name}.`)).catch((error) => setBulkMessage(error instanceof Error ? error.message : 'Could not move the selected cards.')).finally(() => setBulkBusy(false)) } } control.value = '' }}><option value="" disabled>Move to deck</option>{deckNames.map((name) => <option key={name} value={name}>{name}</option>)}</select></label>
        <label className="bulk-due"><span className="visually-hidden">Due date for selected cards</span><input aria-label="Due date for selected cards" type="date" value={bulkDueDate} onChange={(event) => setBulkDueDate(event.target.value)} /><button type="button" disabled={bulkBusy || !bulkDueDate || !selectedCards.length} onClick={() => { setBulkBusy(true); void setCardsDueDate(selectedCards.map((card) => card.id), bulkDueDate).then(() => setBulkMessage(`Rescheduled ${selectedCards.length} selected ${selectedCards.length === 1 ? 'card' : 'cards'}.`)).catch((error) => setBulkMessage(error instanceof Error ? error.message : 'Could not reschedule the selected cards.')).finally(() => setBulkBusy(false)) }}><CalendarClock size={15} /> Set due</button></label>
        {mode === 'notes' && <label className="bulk-tag"><span className="visually-hidden">Tag for selected notes</span><input aria-label="Tag for selected notes" value={bulkTag} onChange={(event) => setBulkTag(event.target.value)} placeholder="tag" /><button type="button" disabled={!bulkTag.trim()} onClick={() => { updateItemsBulk([...selected], { addTags: [bulkTag] }); setBulkTag('') }}>Add tag</button><button type="button" disabled={!bulkTag.trim()} onClick={() => { updateItemsBulk([...selected], { removeTags: [bulkTag] }); setBulkTag('') }}>Remove tag</button></label>}
      ...
      <section className="library-list" aria-label={mode === 'notes' ? 'Notes' : 'Cards'}>
        <div className="library-list-header"><label><input type="checkbox" aria-label={`Select all visible ${mode}`} checked={visibleSelectionIds.length > 0 && visibleSelectionIds.every((id) => selected.has(id))} onChange={(event) => setSelected((current) => { const next = new Set(current); visibleSelectionIds.forEach((id) => event.target.checked ? next.add(id) : next.delete(id)); return next })} /></label><span>{mode === 'notes' ? 'Knowledge' : 'Card'}</span><span>{mode === 'notes' ? 'Practice prompts' : 'Template'}</span><span>Status</span><span className="visually-hidden">Actions</span></div>
        {mode === 'notes' && visibleItems.map((item) => {
          ...
          return (
            <article className="library-row" key={item.id}>
              ...
              <div className="variant-cell">{cards.map((card) => <button key={card.id} className={card.suspended ? 'variant-pill suspended' : 'variant-pill'} onClick={() => toggleSuspend(card.id)} title={card.suspended ? 'Resume prompt' : 'Suspend prompt'}>{card.flags ? <Flag size={12} aria-label={`Flag ${card.flags}`} /> : null}{card.variant}{card.suspended && ' · paused'}{card.buriedUntil && new Date(card.buriedUntil) > new Date() ? ` · ${card.buriedBy === 'user' ? 'user buried' : 'sibling buried'}` : ''}</button>)}</div>
        ...
        {mode === 'cards' && visibleCards.map((card) => {
          ...
          return <article className="library-row card-browser-row" key={card.id}>
            <label className="row-select"><input type="checkbox" aria-label={`Select card ${item.prompt} ${card.variant}`} checked={selected.has(card.id)} onChange={(event) => setSelected((current) => { const next = new Set(current); if (event.target.checked) next.add(card.id); else next.delete(card.id); return next })} /></label>
            <div className="knowledge-cell"><span className="collection-label">{card.deckName || item.collection}</span><strong>{item.prompt}</strong><p>{item.answer}</p><div className="tag-row"><span>Card ID {card.id}</span>{item.noteModel && <span>{item.noteModel.noteTypeName}</span>}</div></div>
            <div className="variant-cell"><button className={card.suspended ? 'variant-pill suspended' : 'variant-pill'} onClick={() => toggleSuspend(card.id)}>{card.flags ? <Flag size={12} aria-label={`Flag ${card.flags}`} /> : null}{card.variant}{card.suspended ? ' · suspended' : ''}</button></div>
            ...
            <div className="row-actions"><button className="icon-button" onClick={() => setEditing(item)} aria-label={`Edit note for ${item.prompt}`}><Edit3 size={18} /></button></div>
          </article>
        })}
        {!visibleTotal && <div className="empty-state"><Archive size={30} /><h2>No {mode} found</h2><p>{healthFilter === 'all' ? 'Try another filter or add a new item.' : 'No matching collection problems were found.'}</p><button className="secondary-button" onClick={() => { setQuery(''); setCollection('All collections'); setHealthFilter('all'); setVisibleCount(LIBRARY_PAGE_SIZE) }}><MoreHorizontal size={18} /> Clear filters</button></div>}
      </section>
      {visibleTotal > (mode === 'notes' ? visibleItems.length : visibleCards.length) && <div className="library-pagination"><p aria-live="polite">Showing {mode === 'notes' ? visibleItems.length : visibleCards.length} of {visibleTotal} matching {mode}.</p><button className="secondary-button" onClick={() => setVisibleCount((count) => Math.min(visibleTotal, count + LIBRARY_PAGE_SIZE))}>Show {Math.min(LIBRARY_PAGE_SIZE, visibleTotal - (mode === 'notes' ? visibleItems.length : visibleCards.length))} more</button></div>}
