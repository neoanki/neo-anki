...
const EditDialog = ({ item, onClose }: { item: KnowledgeItem; onClose: () => void }) => {
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setSaveError(`${error instanceof Error ? error.message : 'Neo Anki could not save this item.'} Your edits are still open; try again.`)
      setSaving(false)
    }
  }
  return (
    <div className="scrim" role="presentation" onMouseDown={onBackdropMouseDown}>
      <form ref={dialogRef} tabIndex={-1} className="edit-dialog" role="dialog" aria-modal="true" aria-labelledby="edit-title" aria-busy={saving} onSubmit={save}>
        ...
        <div className="dialog-actions"><button type="button" className="secondary-button" onClick={requestClose}>Cancel</button><button className="primary-button" type="submit" disabled={!dirty || saving}>{saving ? 'Saving…' : 'Save changes'}</button></div>
        </fieldset>
      </form>
    </div>
  )
