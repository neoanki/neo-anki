...
| Signed/notarized desktop artifacts | Release jobs require Developer ID/notarization and Authenticode credentials and verify trust before upload | Configured; cannot be proven without release credentials and tagged artifacts |
...
- Electron acceptance: all 8 runnable development journeys and the separately invoked exact packaged-launch journey passed against a freshly built unsigned local macOS application. Release workflows still require platform signing/notarization and run the same packaged journey.
...
The product claim remains **not yet a general drop-in replacement**. The remaining claim gates are external or empirical: platform-signed tagged artifacts, real iOS/Android device journeys, hosted-service reliability and recovery exercises, four-persona migration runs with disposable collections, and the comparative UX study defined by the remediation plan. Source implementation or heuristic review cannot substitute for those results.
