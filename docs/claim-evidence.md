...
| Platform releases | Desktop tags build macOS universal, Windows x64, and Linux x64 artifacts with checksums, SBOMs, GitHub attestations, and packaged acceptance checks. macOS additionally requires Developer ID signing, notarization, stapling, and Gatekeeper acceptance. | Windows artifacts remain unsigned; users must verify checksums and GitHub attestations. | Authenticode signing remains a future trust improvement for Windows. |
