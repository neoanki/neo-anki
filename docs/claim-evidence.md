...
Status below is aligned with the July 19 comprehensive audit, its remediation ledger, the v0.2.2 release gates completed July 20, 2026, and the July 21 internal-extension extraction audit. Neo Anki 0.2.3 does not broaden the compatibility, durability, sync, mobile, or learning-effectiveness claims below.
...
| Extension marketplace | The public catalog is pull-request reviewed; desktop pins catalog metadata, hash, publisher key, signed manifest, and capabilities before install. Card Timer 2.0.0, Memory Insights 2.0.0, and NeoAnki TTS 2.0.2 are independently released listings. | Review is not a warranty, identity attestation, or learning-effectiveness claim; browser/mobile are browse-only. Memory Insights exposes a current collection summary, not historical recall analytics. | Listed packages pass catalog validation and signed-release provenance checks; core install/update/rollback coverage remains green. |
...
| Platform releases | Desktop tags build macOS universal, Windows x64, and Linux x64 artifacts with checksums, SBOMs, GitHub attestations, and packaged-launch checks without repository secrets. | macOS and Windows artifacts are unsigned; users must verify checksums and GitHub attestations and follow the documented platform warning flow. | Platform signing and notarization may be added later as optional trust improvements without becoming CI prerequisites. |
