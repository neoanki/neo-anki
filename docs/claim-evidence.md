...
Status below is aligned with the July 19 comprehensive audit, its remediation ledger, and the July 21 completed internal-extension extraction audit. Neo Anki 0.3.0 changes the extension boundary but does not broaden the compatibility, durability, sync, mobile, or learning-effectiveness claims below.
...
| Extension marketplace | The public catalog is pull-request reviewed; desktop pins catalog metadata, hash, publisher key, signed manifest, and capabilities before install. Nine packages are independently released listings, including every former internal extension. | Review is not a warranty, identity attestation, or learning-effectiveness claim; browser/mobile are browse-only. Memory Insights exposes a current collection summary, not historical recall analytics. | Listed packages pass catalog validation and signed-release provenance checks; core install/update/rollback coverage remains green. |
