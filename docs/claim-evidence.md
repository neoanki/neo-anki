...
| Extension marketplace | The public catalog is pull-request reviewed; desktop pins catalog metadata, hash, publisher key, signed manifest, and capabilities before install. NeoAnki TTS 2.0.2 is the first production listing. | Review is not a warranty, identity attestation, or learning-effectiveness claim; browser/mobile are browse-only. | NeoAnki TTS passes catalog validation, signed release provenance checks, and the extension's cross-platform desktop journey; core install/update/rollback coverage remains green. |
